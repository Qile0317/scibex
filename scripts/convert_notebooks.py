...
app = cyclopts.App(help="Convert all Jupyter notebooks from docs directory to HTML in site directory.")
...
def convert_notebooks(
    docs_dir: Annotated[Path, Parameter(help="Path to the docs directory (source)")] = Path("docs"),
    output_dir: Annotated[Path, Parameter(help="Path to the output directory (site)")] = Path("site"),
    *,
    verbose: Annotated[bool, Parameter(help="Print detailed information about conversions")] = False,
):
    """Convert all Jupyter notebooks from docs directory to HTML in site directory."""
    # Check if output directory exists
    if not output_dir.exists():
        print(f"Error: Output directory '{output_dir}' does not exist.", file=sys.stderr)
        print("Please run this after building docs (e.g., after 'just docs-build')", file=sys.stderr)
        return 1
    ...
    if not template_path.exists():
        print(f"Error: Template page not found at {template_path}", file=sys.stderr)
        return 1
    ...
    for notebook_path in notebooks:
        try:
            ...
            if ".ipynb_checkpoints" in str(notebook_path):
                if verbose:
                    print(f"Skipping checkpoint: {notebook_path}")
                skipped_count += 1
                continue
    ...
    if skipped_count > 0:
        print(f"  Skipped {skipped_count} checkpoint file(s)")
